...
void computer_write(void *item, void *file)
{
	FILE *fp = *(FILE**)file;
	Computer *c = item;
...
int computer_eq2(const void *a, const void *b)
{
	const Computer *x = a, *y = b;
	int cmp;

	cmp = strncmp(x->cpu, y->cpu, 21);
	
	if(cmp != 0) {
		return cmp;
	}

	return strncmp(x->gpu, y->gpu, 21);
}
...
void computer_print(const void *c)
{
	const Computer *x = c;
	printf("CPU: %-*s\tGPU: %-*s\tRAM: %-*s\tMIPS: %lu\n", 21, x->cpu, 21, x->gpu, 21,  x->ram, x->mips);
}
